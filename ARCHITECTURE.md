...
`schema/lib.schema` is the single source of truth. `build.rs` runs the
TrueSchema `schema_rust::build::ContractCrateBuild::from_environment` path
with `expect_fresh()`, which regenerates the checked-in `src/schema/lib.rs`
(`// @generated by schema-rust`) and fails the build if the artifact drifts
from the schema. The schema document has six positional roots: `{ imports }`,
`[Input]`, `[Output]`, `{ types }`, `{ generics }`, and `{ impls }`. Empty
optional roots remain explicit. The types block is dotted (`Name.Definition`)
...
ordinary typed exchange output, not a schema-generated subscription frame.
...
- Byte and geometry types: `TerminalInputBytes`, `TerminalTranscriptBytes`,
  ...
  modeled as `(Vector Integer)` — one unsigned integer per byte — because the
...
(`TerminalEvent::TerminalWorkerLifecycleEvent` is carried as
 `Output::Event` on the current exchange envelope.)
```

Closed enums; typed rejection reasons; no string-tagged event kinds.

### Lifecycle observation exchange
...
The request-side retraction operation and reply-side acknowledgement remain
explicit typed contract operations. Streaming lifecycle transport is a runtime
concern; this contract does not encode a stream open/close relation.
...
TerminalWorkerLifecycleRetraction  -> Retract
```
...
| Worker-lifecycle events remain typed outputs without a schema-generated stream transport. | The generated `Frame` is `ExchangeFrame<Input, Output>`; `tests/round_trip.rs` round-trips `Output::Event` as an exchange reply. |
| Subscription retraction is explicit: request-side `TerminalWorkerLifecycleRetraction` carries the token and reply-side `SubscriptionRetracted` echoes it. | The schema names both operations; round-trip witnesses cover the retract request and acknowledgement. |
...
```text
build.rs                   — schema-rust ContractCrateBuild::expect_fresh
schema/
